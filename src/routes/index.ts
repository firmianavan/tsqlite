...
router.get('/get', function (req, res, next) {
    ...
    let n = new Date()
    let ret = `${n.getFullYear()}-${n.getMonth()+1}-${n.getDate()} ${n.getHours()}:${n.getMinutes()}:${n.getSeconds()}`
    console.log(ret)
    res.send(ret)
    // res.write(ret)
...
router.get("/csv/download/:d", function (req, res) {
    ...
    let stmt = db.prepare("select serial as serial_no, m as month, sum(mi)/60 as running_hours   from usr where m = ? group by serial, m", req.params['d'])
